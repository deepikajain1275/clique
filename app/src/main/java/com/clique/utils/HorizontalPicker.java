    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (values != null) {
            for (int i = 0; i < values.length; i++) {
                ...
                if (selectedItem == i) {
                    textPaint.setColor(Color.parseColor("#6551D5"));
                    int w = canvas.getWidth();
                    int h = canvas.getHeight();

                    Paint framePaint = new Paint();
                    framePaint.setStyle(Paint.Style.FILL);
                    framePaint.setColor(Color.parseColor("#6551D5"));
                    canvas.drawCircle(itemWidth/2,h/2,h/2, framePaint);

                    Paint transparentPaint = new Paint();
                    transparentPaint.setColor(Color.WHITE);
                    transparentPaint.setAntiAlias(true);
                    canvas.drawCircle(itemWidth/2,h/2, h/2-5, transparentPaint);
                }
